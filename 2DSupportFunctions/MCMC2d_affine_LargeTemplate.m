function out = MCMC2d_affine_LargeTemplate(init, dataIn, niter, hyperPars)
...
for iter = 1:niter
    ...
    [DTs_log, DTs_exp] = updatesDerivative(Y, Xlatent_cut, b, t0, exp(hyperPars.logsig)/2, iter, floor(niter/5), 1);

    % update transformation
    tupdate1 = transfUpdate_2d_largeX_affine_parallel_mex(t0, DTs_exp, nnIDXs, Xlatent, Y, sigma, alpha, rho,...
    ...
    alpha = alphaUpdate_parallel_mex(Xlatent, XTs, nsubj, Cov_NN, muFsX, 2, 1, parworkers);
    alphaMat(iter) = alpha;
    ...
    if rem(iter,k)==0
        ...
        drawnow
        showShifts(tMat_exp,33, 4, iter)
        drawnow
        showInverseImags(dataIn, tMat_exp, 5, iter)
        drawnow
    end
end
