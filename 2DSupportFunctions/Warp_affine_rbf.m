function out = Warp_affine_rbf(tvec_log, img, coord, logIndicator)
...
nx = length(unique(coord(:,2)));
ny = length(unique(coord(:,1)));

coord_x = reshape(coord(:,2), nx, []);
coord_y = reshape(coord(:,1), ny, []);
