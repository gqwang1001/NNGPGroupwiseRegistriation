function out = sym_transfUpdate_2d_parfor(transf0, invtransf0,  DTs, invDTs, nnIDXs, X, X_cut, Y, sigma,...
    ...
    if jointUpdate == true % jointUpdate
        ...
        if ~checkLogMat(invTmat_prop) && ~checkLogMat(invTmat_prop/(Vec2Mat(invpriors(i,:), false)))
            ...
            invlogliks0(i) = logNormalPdf(Yinv_i(:), b(i)*X_cut, sqrt(sigma(i)))+...
                ...
                (lambda_sym(1)*norm(invt1Mat0*t1Mat0-eye(3), 'fro')+lambda_sym(2)*norm(t1Mat0*invt1Mat0-eye(3), 'fro'));
            ...
            invlogliks1(i) = logNormalPdf(Yinv_i1(:), b(i)*X_cut, sqrt(sigma(i)))+...
                ...
                (lambda_sym(1)*norm(invTmat_prop*(t1Mat0)-eye(3), 'fro')+lambda_sym(2)*norm(t1Mat0*invTmat_prop-eye(3), 'fro'));
%             AP = min(0, invlogliks1(i)-invlogliks0(i)+ logProposalDiff(invTmat_prop, invt1Mat0, invSig(1:6,1:6,i)));
        ...
        if ~checkLogMat(Tmat_prop) && ~checkLogMat(Tmat_prop/(Vec2Mat(priors(i,:), false)))
            ...
            logliks1(i) = logNormalPdf(Y_i(:), b(i)*Covs_NN1(i).mu, sqrt(b(i)^2*Covs_NN1(i).Ft * alpha + sigma(i)))+...
                ...
                (lambda_sym(1)*norm(invt1Mat0*Tmat_prop-eye(3), 'fro')+lambda_sym(2)*norm(Tmat_prop*invt1Mat0-eye(3), 'fro'));
...
%             AP = min(0, logliks1(i)-logliks0(i)+ logProposalDiff(Tmat_prop, t1Mat0, Sig(1:6,1:6,i)));
        ...
        invlogliks0(i) = logNormalPdf(Yinv_i(:), b(i)*X_cut, sqrt(sigma(i)))+...
            logMVTdistPrior(invt1Mat0(:), invpriors(i,:), ta0, tb0, lambda)-...
            (lambda_sym(1)*norm(invt1Mat0*(t1Mat0)-eye(3), 'fro')+lambda_sym(2)*norm(t1Mat0*invt1Mat0-eye(3), 'fro'));
        invprop0 = zeros(1, 6);
        ...
        for j=1:6
            ...
            if ~checkLogMat(invTmat_prop) && ~checkLogMat(invTmat_prop/(Vec2Mat(invpriors(i,:), false)))
                ...
                AP = min(0, invlogliks1(i)-invlogliks0(i) + ...
                    logProposalDiff(invTmat_prop, invt1Mat0, diag(exp(invlogsig(i,1:6))'.*diag(invSig(1:6,1:6,i))), invprop0));
%                 AP = min(0, invlogliks1(i)-invlogliks0(i));
        ...
        for j = 1:6
            ...
            if ~checkLogMat(Tmat_prop) && ~checkLogMat(Tmat_prop/(Vec2Mat(priors(i,:), false)))
                ...
                AP = min(0, logliks1(i)-logliks0(i) + logProposalDiff(Tmat_prop, t1Mat0, diag(exp(logsig(i,1:6))'.*diag(Sig(1:6,1:6,i))), prop0));
%                 AP = min(0, logliks1(i)-logliks0(i));
...
function out = logProposalDiff(T1, T0, Sigma, delta)
out = logProposal(T0, T1, Sigma, -delta)-logProposal(T1, T0, Sigma, delta);
end

function out = logProposal(T1, T0, Sig, delta)
d1given0 = (T1/T0); % T1*inv(T0)
% d1given0 = T0\T1; % inv(T0)*T1

J = detADJ(d1given0);
out = logMVNormalPdf(delta, 0, Sig)+log(abs(1/J));
end
