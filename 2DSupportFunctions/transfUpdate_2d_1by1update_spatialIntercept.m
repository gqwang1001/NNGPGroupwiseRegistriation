function out = transfUpdate_2d_1by1update_spatialIntercept(transf0, nnIDXs, X, Y, sigma, alpha, rho, coord,K, b, nnDict, logsig, Sig, Cov_NN0, incre, nsubj, bnds,rho_i, alpha_i, parworkers)
...
parfor (i = 1:nsubj, parworkers)
    ...
    if exp(tfLogProp)>bnds(1,1) && exp(tfLogProp)<bnds(1,2) 
        ...
        nngp_YminusMu = NNGPmean_var_2d(Y(:,i)-b(i)*Covs_NN1(i).mu, coord, rho_i(i), nnX, (b(i)^2*Covs_NN1(i).Ft*alpha+sigma(i))/alpha_i(i)); 
        logliks1(i) = logNormalPdf(Y(:,i)-b(i)*Covs_NN1(i).mu, nngp_YminusMu.mu, sqrt(nngp_YminusMu.Fs*alpha_i(i)));
    ...
    if exp(tfLogProp)>bnds(1,1) && exp(tfLogProp)<bnds(1,2)
        ...
        A(i) = min(0, logliks1(i)-logliks0(i) +...
            log(normpdf(log(tf1Prop(2)), 0, 1))- log(normpdf(log(tOut(2)), 0, 1)));
